...
장소를 등록해야 합니다. 여행자가 고른 이동수단의 실제 경로로
...
사용자 확인 없이 자동 적용하지 않습니다. 기상청 단기예보는
...
설정합니다. `REVERSE_GEOCODE_URL`과 `FORWARD_GEOCODE_URL`을 비워 두면 뒤쪽
공개 Nominatim 경로가 그대로 남으므로 준비 상태는 `public_shared`로 정식
출시를 차단합니다.

공개 fallback을 제거하는 방법은 둘입니다. 관리형 또는 자체 운영 Nominatim
호환 엔드포인트를 넣거나, 보행 경로·날씨와 같이 `none`·`disabled`·`off` 중
하나를 넣어 **공개 fallback 없음을 명시적으로 선언**하는 것입니다. 후자를
택하면 남는 사슬이 카카오뿐이므로 준비 상태는 `managed`가 되고, 준비 상태
탐침은 카카오 로컬을 인증 헤더와 함께 직접 호출해 응답 계약을 확인합니다.

```bash
# 카카오 단독 운영. 공개 Nominatim 폴백 없음
REVERSE_GEOCODE_URL=none
FORWARD_GEOCODE_URL=none
```

두 값을 선언하지 않으면 카카오가 실제로 그 일을 수행하고 있어도 준비 상태는
공개 fallback을 기준으로 분류되어 `일부 제한`으로 표시됩니다.
...
카카오 호출이 실패하면 공개 Nominatim(사슬에 남아 있는 경우), 그다음 공사
관광정보 최근접 콘텐츠 순으로 내려갑니다. `none`으로 닫아 둔 구성에서는
Nominatim 단계를 건너뜁니다. 각 응답은 어느 제공자가 답했는지
(`source`, `confidence`)를 함께 반환합니다.

### 이동수단별 경로

여행자가 보행·자차·대중교통·자전거 중 하나를 고르면 그 수단의 실제 경로로
이동시간과 복귀시간을 검증합니다. 수단마다 제공자가 다르고, 어느 수단도
다른 수단의 값으로 대체하지 않습니다. 계산에 쓴 제공자는 카드와 출처
원장에 그대로 표시됩니다.

| 수단 | 제공자 | 비고 |
|---|---|---|
| 보행 | TMAP 보행자 경로안내 | 아래 절 참고 |
| 자차·택시 | TMAP 자동차 경로안내 | 출발시각을 `+0900` 형식으로 보내는 경로예측 API |
| 대중교통 | 카카오맵 대중교통 길찾기 | 좌표만 받으므로 소요시간은 조회 시점 시각표 기준 |
| 자전거 | 카카오맵 자전거 길찾기 | |

빈 시간 추천에서는 후보지에서 출발지로 **돌아오는 경로를 따로 조회**해
남은 시간 안에 복귀할 수 있는지까지 확인합니다. 다만 보행은 예외로, 가는
경로를 되짚어 씁니다 — 서울·대전·부산 6개 구간을 양방향으로 실측했을 때
TMAP 보행 경로는 12분짜리부터 80분짜리까지 소요시간이 초 단위까지 같았기
때문입니다(편차 0.0%). 자동차(평균 33.6%)와 자전거(평균 21.4%)는 방향에
따라 크게 달라지므로 되짚어 쓰지 않고 반드시 따로 조회합니다.

카카오 경로 API는 호출 한도를 초과해도 429가 아니라 **HTTP 400** 본문에
`API limit has been exceeded`를 담아 돌려줍니다. 이 응답은 "경로가 없다"와
구분해 재시도하며, 요청 간격을 두어 한도에 닿지 않게 합니다.
