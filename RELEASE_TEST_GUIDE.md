...
- axe의 WCAG 2 A·AA serious/critical 위반과 Lighthouse 색 대비
  위반이 없어야 합니다. `npx --yes @lhci/cli@0.15.1 autorun` 결과에서
  접근성 0.95, 모범사례 0.90, SEO 0.90 미만이면 출시하지 않습니다.
...
- `/sitemap.xml`에는 `/flow`, `/policy`, `/sources`, `/privacy`,
  `/terms`, `/accessibility`가 포함돼야 하고 `/robots.txt`는 API와
