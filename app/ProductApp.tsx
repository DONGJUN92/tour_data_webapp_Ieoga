...
type TabId = "recover" | "insights" | "transparency";
type Incident = "rain" | "delay" | "crowd" | "less_walk";
type Audience = "general" | "stroller" | "wheelchair" | "senior";
type LoadState = "idle" | "loading" | "success" | "error";
type LocationMode = "unselected" | "automatic" | "manual";
type RecoveryOutcome = "idle" | "applied" | "arrived" | "not_arrived";
type Language = "ko" | "en";

const GUIDE_STORAGE_KEY = "ieoga-simulation-guide-seen-v1";
...
const INCIDENTS: { value: Incident; title: string; description: string; marker: string }[] = [
  {
    value: "rain",
    title: "비·기상 변화",
...
const INCIDENTS_EN: Record<Incident, { title: string; description: string }> = {
  rain: {
...
const AUDIENCES_EN: Record<Audience, string> = {
...
const POLICY_APIS = OPEN_APIS.slice(4);
...
function stopTypeFromTourismContent(
  contentTypeId?: string,
): JourneyStop["type"] {
...
function emptyJourneyDraft(): JourneyPlan {
  return {
    id: "new-journey",
    title: "오늘의 여행",
    date: todayInKorea(),
...
function practiceJourneySchedule(): {
  date: string;
  firstTime: string;
  fixedTime: string;
} {
...
function inferRecoveryContext(plan: JourneyPlan): {
...
function normalizeJourneyPlan(payload: unknown): JourneyPlan | null {
...
function normalizeJourneyExecution(payload: unknown): JourneyExecution | null {
  const root = asRecord(payload);
  const execution =
...
function itineraryNodeStart(date: string, time: string): string | undefined {
  if (!date || !time) return undefined;
  return `${date}T${time}:00+09:00`;
}

function itineraryContract(
  plan: JourneyPlan,
...
function normalizePlaceResults(payload: unknown): PlaceSearchResult[] {
...
function statusTone(value: unknown): "good" | "warn" | "bad" | "idle" {
...
function sourceStatus(source: unknown): string {
  const record = asRecord(source);
  return readText(record, ["status", "result", "availability"]) || "사용";
}

function sourceDecisionEffect(source: unknown): string {
...
export function ProductApp() {
  ...
  async function saveJourney(event: FormEvent<HTMLFormElement>) {
    ...
    if (stopWithoutLocation) {
      setJourneyError(
        `‘${stopWithoutLocation.title}’을(를) 장소검색 결과에서 선택해 주세요.`,
      );
      return;
    }
      ...
      (position) => {
        ...
        void fetchJson("/api/v1/location/resolve", {
          ...
          .then((payload) => {
            ...
            setGeoState("success");
            setGeoMessage(
              `${[areaName, districtName].filter(Boolean).join(" ") || "현재 지역"}으로 자동 입력했습니다. 정확한 좌표는 복구 계산에만 사용합니다.`,
            );
          })
          .catch(() => {
            setLocationMode("manual");
  ...
  function selectOriginPlace(place: PlaceSearchResult) {
    ...
    setGeoMessage(
      `현재 위치를 ‘${place.title}’(으)로 설정했습니다.${place.retention === "ephemeral" ? " 이 좌표는 일정에 저장하지 않습니다." : ""}`,
    );
  }
    ...
    >
      ...
      <header className="product-header">
        <a className="product-brand" href="/" aria-label="이어가 홈">
          ...
          <span>
            <strong>이어가</strong>
            <small>IEOGA · TRAVEL CONTINUITY</small>
          </span>
        </a>
          ...
          >
            ...
            <div className="hero-grid">
              <div className="hero-copy">
                ...
                <p>
                  {language === "en" ? (
                    <>
                      When rain, delay, crowds or mobility barriers disrupt your day,
                      IEOGA changes only the broken segment and
                      <strong> protects your next fixed reservation.</strong>
                    </>
                  ) : (
                    <>
                      비, 지연, 혼잡, 이동 부담이 생기면 깨진 구간만 바꾸고
                      <strong> 다음 예약과 여행 목적을 지키는 복구안</strong>을 찾습니다.
                    </>
                  )}
                </p>
              </div>
            ...
            <ol className="journey-steps" aria-label="이어가 사용 단계">
              ...
              <li className={appliedOptionId ? "is-current" : ""}>
                ...
                <span>
                  ...
                  <small>{language === "en" ? "Prove continuity" : "여행 연속성 증명"}</small>
                </span>
              </li>
            </ol>
            ...
            <div className="service-grid" hidden={!journeyPlan || journeyEditing}>
              ...
              >
                ...
                <fieldset className="form-group">
                  <legend>자동으로 지키는 조건</legend>
                  <div className="derived-time-card">
                    <span>다음 예약을 지키기 위한 복구 가능 시간</span>
                ...
                <p className="estimate-note">
                  이동 구간별 실제 경로 근거와 운영 여부를 확인한 후보만 결과에 표시합니다.
                  확인할 수 없는 조건은 숨기지 않고 별도로 표시합니다.
                </p>
              </form>
              ...
              >
                <div className="panel-heading dark">
                  <span>02</span>
                  <div>
                    <p>CONTINUITY OPTIONS</p>
                    <h2>원래 일정으로 돌아가는 복구안</h2>
                  </div>
                </div>
                ...
                {recoverState === "idle" && (
                  <div className="result-empty">
                    ...
                    <strong>조건을 입력하면 이곳에 결과가 나타납니다.</strong>
                    <p>
                      후보를 먼저 많이 보여주지 않습니다. 시간·거리·접근성 조건을 통과한 실제 관광정보만
                      제안합니다.
                    </p>
                  </div>
                )}
                ...
                {recoverState === "loading" && (
                  <div className="result-loading" role="status">
                    ...
                    <strong>전국 관광데이터를 교차 확인하고 있습니다.</strong>
                    <p>조건을 어긴 후보는 이 단계에서 제외됩니다.</p>
                  </div>
                )}
                ...
                {recoverState === "success" && recovery && recovery.options.length > 0 && (
                  <div className="recovery-results">
                        ...
                        >
                          ...
                          <div className="option-content">
                            <div className="option-title-row">
                              <div>
                                {option.strategyLabel && (
                                  <span className="strategy-label">{option.strategyLabel}</span>
                                )}
                            ...
                            <div className="option-footer">
                              <span>운영 여부: {compactValue(option.availability)}</span>
                    ...
                    <details className="decision-contribution">
                      <summary>왜 이 복구안이 안전한지 확인</summary>
                      <div className="decision-contribution-intro">
                        <span>WHY THIS PLAN SURVIVED</span>
                        <h3>각 데이터가 바꾼 판단</h3>
                        <p>데이터 이름보다, 이 복구안이 남거나 제외되는 데 한 역할을 보여줍니다.</p>
                      </div>
                      <ul>
        ...
        {activeTab === "transparency" && (
          ...
          >
            <div className="section-intro">
              ...
              <h1>추천보다 먼저, 근거를 공개합니다.</h1>
              <p>
                이어가는 한국관광공사 OpenAPI의 응답과 명시된 규칙만 사용합니다. 연결이 끊기거나
                확인할 수 없는 조건은 추정값으로 메우지 않고 사용자에게 그대로 알립니다.
              </p>
            </div>
            ...
            {healthError && (
              <div className="notice is-error" role="alert">
                <strong>준비 상태를 확인하지 못했습니다.</strong>
                <p>{healthError}</p>
              </div>
            )}
            ...
            <div className="api-board" data-testid="health-source-list">
              <div className="board-heading">
                <div>
                  <p>공식 데이터 연결</p>
                  <h2>한국관광공사 OpenAPI 8종</h2>
                </div>
                <span>원문 응답 미저장 · 파생 근거만 기록</span>
              </div>
      ...
      <nav className="mobile-nav" aria-label="모바일 주요 메뉴">
        <button className={activeTab === "recover" ? "is-active" : ""} onClick={() => changeTab("recover")}>
          <span aria-hidden="true">↗</span>
          여행 복구
        </button>
        <button className={activeTab === "insights" ? "is-active" : ""} onClick={() => changeTab("insights")}>
          <span aria-hidden="true">▦</span>
          개선 미션
        </button>
        <button
